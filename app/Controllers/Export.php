...
class Export extends BaseController
{
    ...
    public function export_surat_keluar_pdf()
    {
        ...
        if (empty($tanggal_awal) && empty($tanggal_akhir)) {
            $filteredData = $this->suratKeluarModel->getAll($nama_klien, $progres);
            $periode = "Semua Data";
        } else {
            $filteredData = $this->suratKeluarModel->getFiltered($tanggal_awal, $tanggal_akhir, $nama_klien, $progres);
            $periode = 'Periode ' . tanggal_indo($tanggal_awal) . ' s/d ' . tanggal_indo($tanggal_akhir);
        }
    ...
    <div class="pdf-header" style="text-align: center; margin-bottom: 20px;">
        ...
        <tr>
            <td>' . $no . '</td>
            <td>' . tanggal_indo($data->tgl_surat) . '</td>
            <td>' . $data->no_surat . '</td>
            <td>' . $data->nama_klien . '</td>
            <td>' . $data->perihal . '</td>
            <td>' . $data->progres . '</td>
        ...
        return $this->response
            ->setHeader('Content-Type', 'application/pdf')
            ->setHeader('Content-Disposition', 'attachment; filename="Laporan Surat Keluar.pdf"')
            ->setBody($pdfContent);
    }
    public function export_surat_keluar_excel()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $sheet->setCellValue('A4', 'No')
            ->setCellValue('B4', 'Tanggal Surat')
            ->setCellValue('C4', 'No Surat')
            ->setCellValue('D4', 'BPR/Klien')
            ->setCellValue('E4', 'Perihal')
            ->setCellValue('F4', 'Progres');


        // Ambil data berdasarkan kondisi filter
        // Ambil data
        if (!empty($tanggal_awal) && !empty($tanggal_akhir)) {
            $query = $this->suratKeluarModel->getFiltered($tanggal_awal, $tanggal_akhir, $nama_klien, $progres);
        } else {
            $query = $this->suratKeluarModel->getAll($nama_klien, $progres);
        }
        ...
        foreach ($query as $data) {
            ...
            $sheet->setCellValue('D' . $row, $data->nama_klien);
            $sheet->setCellValue('E' . $row, $data->perihal);
            $sheet->setCellValue('F' . $row, $data->progres);
